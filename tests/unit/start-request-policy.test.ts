...
import { canonicalNativeRecovery, validateExecutable } from '../../electron/start-request-policy'
...
describe('start request policy', () => {
  ...
  it('accepts only exact native resume arguments and rebuilds recovery from the start executable', () => {
    expect(canonicalNativeRecovery('codex', 'native-1', 'codex', ['resume', 'native-1'], {
      executable: 'codex', args: ['resume', 'native-1'],
    })).toEqual({ executable: 'codex', args: ['resume', 'native-1'] })
    expect(() => canonicalNativeRecovery('codex', 'native-1', 'codex', ['resume', 'native-1', '--extra'], {
      executable: 'codex', args: ['resume', 'native-1'],
    })).toThrow(/resume/i)
